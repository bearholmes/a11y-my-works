...
export function Licenses() {
  ...
  return (
    <div className="m-5">
      ...
      {!error ? (
        <div>
          <pre className="whitespace-pre-wrap font-mono text-sm text-zinc-950 dark:text-zinc-50 bg-zinc-50 dark:bg-zinc-900 p-4 rounded border border-zinc-200 dark:border-zinc-800 overflow-x-auto max-h-[600px] overflow-y-auto">
            {licenseText}
          </pre>
        </div>
      ) : (
        <Text className="mt-2">
          라이센스 정보를 확인할 수 없습니다. 관리자에게 문의하세요.
        </Text>
      )}
    </div>
  );
